<?php
// Amadou 3.x
// $Id: template.php,v 1.1 2008/07/08 07:54:41 intertextual Exp $
...
*/
function amadou_get_mainContent_width($sidebar_right) {
  $width = 650;
  if (!$sidebar_right) {
    $width = $width + 300;
  }  
...
function amadou_get_sideBars_width($sidebar_right) {
  $width = 0;
...
$output = '';
  ...
  if (count($links) > 0) {
    ...
    foreach ($links as $key => $link) {
      ...
      if (isset($link['href'])) {
        $output .= l(t($link['title']), $link['href'],
        	array(
        ...
        if (!$html) {
          $link['title'] = check_plain(t($link['title']));
        }
        $output .= '<span'. drupal_attributes($link['attributes']) .'>' . t($link['title']) .'</span>';
      }
